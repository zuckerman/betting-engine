...
import { isBetfairLive } from "@/lib/betfair-odds-service"
...
async function verifyTransition() {
  console.log("🔍 VALIDATION SWITCH VERIFICATION\n")
  console.log("=" .repeat(60))

  // 1. Check if Betfair credentials configured
  console.log("\n1️⃣  BETFAIR CREDENTIALS STATUS:")
  const liveMode = isBetfairLive()
  console.log(`   Mode: ${liveMode ? "🟢 REAL ODDS" : "🟡 MOCK ODDS"}`)
  console.log(
  ...
  if (!settled || settled.length === 0) {
    ...
    settled.forEach((pred, idx) => {
      ...
      console.log(`   [${idx + 1}] ${pred.match_id || "N/A"}`)
      ...
      if (typeof closingOdds === "number") {
        ...
        const dataSource = liveMode && isRealistic ? "🟢 REAL" : "🟡 MOCK"
        console.log(`       Data Source: ${dataSource}`)
      }

      console.log(`       CLV: ${typeof clv === "number" ? `${(clv * 100).toFixed(2)}%` : clv}`)
      console.log()
    })
  }
  ...
  if (allPreds && allPreds.length > 0) {
    ...
    console.log(`   Entry Odds Range: ${entryOdds[0].toFixed(2)} - ${entryOdds[entryOdds.length - 1].toFixed(2)}`)
    console.log(
